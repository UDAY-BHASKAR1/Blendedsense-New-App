...
export class DashboardSweepblocksComponent {
  ...
  Add() {
    let EquippmentQuantity = this.testForm.get(
      'EquippmentQuantity'
    ) as FormArray;
  ...
  deleteSweep() {
    ...
    this.http.deleteSweep(this.globalId).subscribe((res) => {
      ...
      this.toastr.success('Sweepblock deleted successfull', null, {
        timeOut: 1000,
      });
  ...
  EditData(data) {
    ...
    let sweepObject = this.sweepTypeoptions.filter(
      (sweep) => sweep.id === data.sweepBlockTypeId
    );
    this.displayBasic2 = true;
  ...
  saveSweep(id) {
    ...
    if (this.id) {
      body['id'] = this.id;
      this.http.Editsweep(body, id).subscribe((res) => {
        this.getSweeps();
      });
    } else {
      this.http.Addsweep(body).subscribe((res) => {
        this.getSweeps();
      });
    }
  }
  ...
  dropdown1(selectedSweepType): void {
